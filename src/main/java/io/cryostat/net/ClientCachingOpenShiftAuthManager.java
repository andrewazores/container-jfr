...
public class ClientCachingOpenShiftAuthManager extends AbstractAuthManager {

    static final Duration DEFAULT_TTL = Duration.ofSeconds(90);

    private final FileSystem fs;
    private final LoadingCache<String, OpenShiftClient> clients;

    public ClientCachingOpenShiftAuthManager(Logger logger, FileSystem fs, Duration ttl) {
        super(logger);
        this.fs = fs;
        ...
        this.clients =
                Caffeine.newBuilder()
                        ...
                        .removalListener(
                                new RemovalListener<String, OpenShiftClient>() {
                                    @Override
                                    public void onRemoval(
                                            String token,
                                            OpenShiftClient authClient,
                                            RemovalCause cause) {
                                        if (token == null) {
                                            logger.warn(
                                                    "AuthClient eviction triggered with null token");
                                            return;
                                        }
                                        if (authClient == null) {
                                            logger.warn(
                                                    "AuthClient eviction triggered with null client");
                                            return;
                                        }
                                        logger.info("Removing cached authClient for {}", token);
                                        authClient.close();
                                    }
                                })
                        .build(
        ...
        return CompletableFuture.supplyAsync(
                        () -> {
                            ...
                            try {
                                ...
                                clients.get(token).routes().inNamespace(getNamespace()).list();
    ...
    @Name("io.cryostat.net.ClientCachingOpenShiftAuthManager.AuthRequest")
    @Label("AuthRequest")
    @Category("Cryostat")
    @SuppressFBWarnings(
